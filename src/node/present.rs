...
use {
    ...
    chain::{
        chain::{BufferChains, ImageChains}, 
        schedule::Submission,
        pass::{StateUsage, Pass, PassId},
        resource::State,
        sync::SyncData,
    },
    hal::{
        Backend, Device,
        image, buffer,
        command::{CommandBufferFlags, ImageCopy, RawLevel, RawCommandBuffer},
        format::{ChannelType, Format},
        queue::{RawSubmission, RawCommandQueue, QueueFamily},
        pool::{CommandPoolCreateFlags, RawCommandPool},
        pso::PipelineStage,
        window::{Swapchain, Backbuffer, Surface, FrameSync, SwapchainConfig},
    },
    smallvec::SmallVec,
    node::low::{AnyNodeBuilder, AnyNode},
    util::*,
};
    ...
    B: Backend,
{
        ...
        I: Borrow<B::Image>,
    {
        ...
        let (swapchain, backbuffer) = device.create_swapchain(self.surface, SwapchainConfig {
        ...
        let per_frame = match backbuffer {
            Backbuffer::Images(ref images) => {
                ...
                cbufs.into_iter().enumerate().map(|(index, mut cbuf)| {
                    cbuf.begin(CommandBufferFlags::EMPTY, Default::default());
                    cbuf.copy_image(
                        resource.image.borrow(),
                        chains[&self.id.0].link(submission.image(self.id.0)).state().layout,
                        &images[index],
    ...
    I: Borrow<B::Image>,
{
    ...
    fn pass(
        ...
        Pass {
            id,
            family: families.iter().find(|qf| self.surface.supports_queue_family(qf)).unwrap().id(),
            queue: None,
            dependencies: Vec::new(),
            buffers: HashMap::new(),
            images: once((self.id.0, StateUsage {
                state: State {
    ...
    fn build(
        ...
        Box::new(
            self.build_node(submission, chains, resources, family, device)
        )
    }
}
    ...
    D: Device<B>,
{
    fn run<'a>(
        ...
        let wait = sync.acquire.wait.iter().map(|wait| (&semaphores[*wait.semaphore()], wait.stage()));
        let acquire = self.free.take().unwrap();
        let frame = self.swapchain.acquire_frame(FrameSync::Semaphore(&acquire)).id();
        ...
        let submission = RawSubmission {
            wait_semaphores: &wait.chain(Some((acquire, PipelineStage::TRANSFER))).collect::<SmallVec<[_; 8]>>(),
